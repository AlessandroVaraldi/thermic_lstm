...
def train_model(
    ...
    warmup_epochs: int = LAMBDA_WARMUP_EPOCHS,       # NEW
    device: str = "cpu",
    use_amp: bool = True,
    ...
    first_ds = (train_dl.dataset.datasets[0]                     # ConcatDataset
            if hasattr(train_dl.dataset, "datasets")
            else train_dl.dataset)                            # WindowDataset
    mu_y, std_y = float(first_ds.mu_y), float(first_ds.std_y)
    ...
    try:
        for epoch in range(1, MAX_EPOCHS + 1):
            ...
            if warmup_epochs > 0:
                ...
                scale = 1.0  # scheduler disabilitato

            lambda_ss_eff = lambda_ss * scale
            lambda_tr_eff = lambda_tr * scale
            ...
            for xb, yb in train_dl:
                ...
                with autocast(device_type="cuda", enabled=use_amp):
                    y_pred = model(xb)
                    y_pred_deg = mu_y + std_y * y_pred    # (B,)  °C
                    ...
                    if lambda_ss_eff > 0:
                        ...
                        loss_ss = criterion(y_pred_deg, T_ss)

                    xb_prev    = torch.cat([xb[:, :1, :], xb[:, :-1, :]], 1)
                    T_prev     = model(xb_prev)                  # normalizzato
                    T_prev_deg = mu_y + std_y * T_prev           # °C

                    res_tr = transient_residual(
                        T_prev_deg, y_pred_deg,                  # °C
                        P_p, P_c,
                        Tbp_p, Tbp_c,
                    ...
                    loss = loss_data + lambda_ss_eff * loss_ss + loss_tr * lambda_tr_eff
            ...
            log.info("[%03d] train=%.6f  val=%.6f  (best %.6f)", epoch, avg_train, cur_val, best_val)
    ...
    plt.title(
        "Physics-Informed Training" if lambda_ss_eff > 0 else "Data-Driven Training"
    )
    plt.legend(); plt.grid(True); plt.tight_layout()

    fname = ("plots/phys_informed_loss_curves.png"
             if lambda_ss_eff > 0 else
